...
// максимальное рассто€ние на котором можно использовать объекты
...
{
	// перезаписываем функцию UTPlayerController, чтобы не слышать "Play!" при старте каждого уровн€
}

reliable client function PlayStartupMessage(byte StartupStage)
{
	// перезаписываем функцию UTPlayerController, чтобы не видеть приветстви€ при старте каждого уровн€
}
