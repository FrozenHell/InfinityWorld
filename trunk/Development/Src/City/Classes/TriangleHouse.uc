...
// пр€моугольные блоки здани€
var array<MyHouse> Blocks;

// треугольные части здани€
var array<TestTriFloor> TriParts;
// треугольные части дл€ крыши
...
function DrawCenter(float posX, float posY, float floor, float angle, Myhouse block1, Myhouse block2, Myhouse block3, int unormalBranch)
{
	...
	// убираем варнинги компил€тора
	localNode3 = None;
	localNodes[0] = None;

	posZ = floor * HeiW;

	// ставим центральную часть дл€ этажа
	locPos.x = posX;
	locPos.y = posY;
	locPos.z = posZ;
	locPos += Location;
	...
	for (i = 1; i <= Width; i++)
		for (j = Width - i; j < Width; j++)
		{
			...
			// если это не крайний р€д, то добавл€ем дополнительный блок
			if (i < Width)
			{
			...
			else // если это крайний р€д, то св€зываем с соседним домом
			{
				// добавл€ем св€зи с соседним зданием
...
// св€зать два узла двусторонней св€зью
static function BindNodes(NavNode A, NavNode B)
{
	...
	{
		if (A == None)
			`warn("попытка св€зать с несуществующей нодой A");
		else
			`warn("попытка св€зать с несуществующей нодой B");
	}
}

function DrawTriHousePart(float posX, float posY, float angle, int type, int next, int deep = 0, optional MyHouse prevBranch) // если есть кос€ки, уберите optional и откомпилируйте
{
	local int i;
	local MyHouse block1, block2, block3;
	// номер ветки, котора€ считаетс€ с обратного конца (-1 если такой нет)
	...
	// дл€ всех этажей строим центральную часть
	for (i = 0; i < Height; i++)
